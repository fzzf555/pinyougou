...
public class TypeTemplateServiceImpl implements TypeTemplateService {
	...
	private void saveToRedis(){
		...
		for(TbTypeTemplate tbTypeTemplate : list){
			//缓存品牌列表
			List typeTemplate = JSON.parseArray(tbTypeTemplate.getBrandIds(),Map.class);
			redis.boundHashOps("brandList").put(tbTypeTemplate.getId(),typeTemplate);
			//缓存规格列表
			List<Map> specList = findSpecList(tbTypeTemplate.getId());
			redis.boundHashOps("specList").put(tbTypeTemplate.getId(),specList);
		}
	...
	public List<Map> findSpecList(Long id) {
		...
		//使用fastjson将获取到的字符串转换成指定格式的list集合
